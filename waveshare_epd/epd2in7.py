...
from .screen_lut import Lut, Gray_Lut
...
class EPD:
    ...
    def __gray_set_lut(self):
        self.send_command(0x20)
        for x in Gray_Lut.gray_lut_vcom:  # vcom
            self.send_data(x)
        self.send_command(0x21)  # red not use
        for x in Gray_Lut.gray_lut_ww:
            self.send_data(x)
        self.send_command(0x22)  # bw r
        for x in Gray_Lut.gray_lut_bw:
            self.send_data(x)
        self.send_command(0x23)  # wb w
        for x in Gray_Lut.gray_lut_wb:
            self.send_data(x)
        self.send_command(0x24)  # bb b
        for x in Gray_Lut.gray_lut_bb:
            self.send_data(x)
    ...
    def display_4Gray(self, image:Image):
